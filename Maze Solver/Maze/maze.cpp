...
Maze::Maze( int _width, int _height)
{
	...
	this->pMazeData = new int[ this->width * this->height ];
	this->pMazeCellInfo = new MazeInfo[this->width * this->height];
	memset(pMazeData,0,sizeof(bool)*this->width * this->height);
	assert( pMazeData != 0 );
}
...
void Maze::Load( const char * const inFileName )
{
	...
	//My additon for cell info
	this->pMazeCellInfo = new MazeInfo[this->width * this->height];
	//memset(pMazeData,0x0,sizeof(bool)*this->width * this->height);
	this->pMazeParentData = new Parent [this->width * this->height];
...
//	printf("  ---> start (row,col)  %d %d \n",at.row, at.col);
	...
	for( iter = soln->begin(); iter != soln->end(); ++iter ) 
	{
		...
		at = at.move(dir);
		//printf("%s-", DirectionHelper::getString(*iter));
		//printf("(row,col)  %d %d \n",at.row, at.col);
	}
...
MazeInfo Maze::getMazeCellInfo(Position pos)
{
	MazeInfo val = pMazeCellInfo[pos.col + pos.row * this->width];
	return val;
}
void Maze::setMazeCellInfo(Position pos,MazeThread thread)
{

	pMazeCellInfo[pos.col + pos.row * this->width].isPassed=true;
	pMazeCellInfo[pos.col + pos.row * this->width].passedThread=thread;
}

Parent Maze::getParentInfo(Position pos)
{
	
	Parent val =pMazeParentData[pos.col + pos.row * this->width];
	return val;
}

void Maze::setParentInfo(Position pos,Position _parent,Direction _dir)
{
	int mypos = pos.col + pos.row * this->width;
	int parentpos = _parent.col + _parent.row * this->width;
	pMazeParentData[pos.col + pos.row * this->width].parentPos = _parent;
	pMazeParentData[pos.col + pos.row * this->width].path = _dir;
